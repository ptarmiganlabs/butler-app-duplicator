...
try {
    qrsInstanceCustomPropertyCheck1
                ...
                qrsInstanceCustomPropertyCheck1
                    .Post(
                        'custompropertydefinition',
                        {
                            ...
                            choiceValues: ['Yes'],
                            description:
                                'When set to yes, the associated app will be used as a template app',
                            objectTypes: ['App'],
                            valueType: 'Text',
                            privileges: null,
                        },
                    ...
                    .then(result => {
                        if (result.statusCode == 201) {
                            logger.verbose(
                                `Success - new custom property created: ${config.get(
                                    'customPropertyName',
                                )}`,
                            );
                            ...
                            logger.debug(
                                `ID of app-is-created-from-template custom property: ${result.body.id}`,
                            );
                        }
                    });
            } else {
                logger.verbose(
                    `Needed custom property already exsits: ${config.get('customPropertyName')}`,
                );
            }
        });
    ...
    logger.error(`APP-IS-TEMPLATE: error ${error}`);
}
...
qrsInstanceCustomPropertyCheck2
    .Get('custompropertydefinition')
    .then(result => {
    ...
    .then(customPropertyExists => {
        if (customPropertyExists == false) {
            // The needed custom property does not exist. Create it.
            logger.verbose(
                `Creating new custom property: ${config.get('customPropertyCreatedFromTemplate')}`,
            );
            qrsInstanceCustomPropertyCheck2
                .Post(
                    'custompropertydefinition',
                    {
                        ...
                        choiceValues: ['Yes'],
                        description:
                            'When set to yes, the associated app was created from a template app',
                        objectTypes: ['App'],
                        valueType: 'Text',
                        privileges: null,
                    },
                ...
                .then(result => {
                    if (result.statusCode == 201) {
                        logger.verbose(
                            `Success - new custom property created: ${config.get(
                                'customPropertyCreatedFromTemplate',
                            )}`,
                        );
            ...
            logger.verbose(
                `Needed custom property already exsits: ${config.get(
                    'customPropertyCreatedFromTemplate',
                )}`,
            );
        }
...
function respondGetTemplateList(req, res, next) {
    ...
    qrsInteractInstance
        .Get(`app/full?filter=@${config.get('customPropertyName')} eq 'Yes'`)
        .then(result => {
...
function respondDuplicateNewScript(req, res, next) {
    ...
    request.get(loadScriptURL, function(error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            qrsInteractInstance
                .Get(`app/${req.query.templateAppId}`)
                .then(result => {
                    logger.verbose(
                        `${req.query.templateAppId}: Testing if specified template app really is a template`,
                    );
                    ...
                    result.body.customProperties.forEach(function(item) {
                        ...
                        if (
                            item.definition.name == config.get('customPropertyName') &&
                            item.value == 'Yes'
                        ) {
                            appIsTemplate = true;
                        }
                    ...
                    if (!appIsTemplate) {
                        ...
                        next(
                            new errors.InvalidArgumentError(
                                'The provided app ID does not belong to a template app.',
                            ),
                        );
                    }
                ...
                .then(result => {
                    ...
                    if (result) {
                        qrsInteractInstance
                            ...
                            .then(result => {
                                logger.info(
                                    `App created with ID ${result.body.id}, using ${req.query.templateAppId} as a template`,
                                );
                                ...
                                qrsInteractInstance
                                    ...
                                    .then(result => {
                                        ...
                                        enigma
                                            ...
                                            .then(global => {
                                                ...
                                                // Connect to engine
                                                logger.verbose(
                                                    `${req.query.appName}: Connecting to engine...`,
                                                );
                                                ...
                                                g.openDoc(newAppId)
                                                    .then(a => {
                                                        ...
                                                        app.setScript(loadScript)
                                                            .then(result => {
                                                                ...
                                                                if (reloadNewApp) {
                                                                    logger.verbose(
                                                                        `${req.query.appName}: Reloading app (might take a while, depending on the app)...`,
                                                                    );
                                                                    app.doReload()
                                                                        .then(result => {
                                                                            logger.verbose(
                                                                                `${req.query.appName}: App reloaded.`,
                                                                            );
                                                                            ...
                                                                            app.doSave()
                                                                                .then(result => {
                                                                                    ...
                                                                                    logger.verbose(
                                                                                        `${req.query.appName}: Close connection to engine...`,
                                                                                    );
                                                                                    g.session
                                                                                        .close()
                                                                                        .then(
                                                                                            () => {
                                                                                                logger.verbose(
                                                                                                    `${req.query.appName}: Connection closed...`,
                                                                                                );
                                                                                                logger.info(
                                                                                                    `${req.query.appName}: Done duplicating, new app id=${newAppId}`,
                                                                                                );
                                                                                                var jsonResult = {
                                                                                                    result:
                                                                                                        'Done duplicating app (new app was reloaded)',
                                                                                                    newAppId: newAppId,
                                                                                                };
                                                                                        ...
                                                                                        .catch(
                                                                                            err => {
                                                                                                // Return error msg
                                                                                                logger.error(
                                                                                                    `Duplication error 1: ${err}`,
                                                                                                );
                                                                                                next(
                                                                                                    new errors.BadRequestError(
                                                                                                        'Error occurred when closing newly created app.',
                                                                                                    ),
                                                                                                );
                                                                                                return;
                                                                                            },
                                                                                ...
                                                                                .catch(err => {
                                                                                    ...
                                                                                    next(
                                                                                        new errors.BadRequestError(
                                                                                            'Error occurred when saving newly created app.',
                                                                                        ),
                                                                                    );
                                                                                    return;
                                                                                });
                                                                        ...
                                                                        .catch(err => {
                                                                            // Return error msg
                                                                            logger.error(
                                                                                `Duplication error (during reload): ${err}`,
                                                                            );
                                                                            next(
                                                                                new errors.BadRequestError(
                                                                                    'Error occurred when reloading newly created app.',
                                                                                ),
                                                                            );
                                                                            return;
                                                                        });
                                                                    ...
                                                                    logger.verbose(
                                                                        `${req.query.appName}: App reloading disabled - skipping.`,
                                                                    );
                                                                    app.doSave()
                                                                        .then(result => {
                                                                            // Close our connection.
                                                                            ...
                                                                            g.session
                                                                                ...
                                                                                .then(() => {
                                                                                    ...
                                                                                    var jsonResult = {
                                                                                        result:
                                                                                            'Done duplicating app (new app was not reloaded)',
                                                                                        newAppId: newAppId,
                                                                                    };
                                                            ...
                                                            .catch(err => {
                                                                // Return error msg
                                                                logger.error(
                                                                    `Duplication error 2: ${err}`,
                                                                );
                                                                next(
                                                                    new errors.BadRequestError(
                                                                        'Error occurred when replacing script of newly created app.',
                                                                    ),
                                                                );
                                                                return;
                                                            });
                                                    ...
                                                    .catch(err => {
                                                        ...
                                                        next(
                                                            new errors.BadRequestError(
                                                                'Error occurred when opening newly created app.',
                                                            ),
                                                        );
                                                        return;
                                                    });
                                            ...
                                            .catch(err => {
                                                ...
                                                next(
                                                    new errors.BadRequestError(
                                                        'Error occurred when creating Enigma object.',
                                                    ),
                                                );
                                                return;
                                            });
                                    ...
                                    .catch(err => {
                                        ...
                                        next(
                                            new errors.BadRequestError(
                                                'Error occurred when setting custom property.',
                                            ),
                                        );
                                        return;
                                    });
                            ...
                            .catch(err => {
                                ...
                                next(
                                    new errors.BadRequestError(
                                        'Error occurred when creating new app from template.',
                                    ),
                                );
                                return;
                            });
                ...
                .catch(err => {
                    ...
                    next(
                        new errors.BadRequestError('Error occurred when test app template status.'),
                    );
                    return;
                });
        } else {
...
function respondDuplicateKeepScript(req, res, next) {
    ...
    qrsInteractInstance
        ...
        .then(result => {
            logger.verbose(
                `${req.query.templateAppId}: Making sure the specified template app really is a template`,
            );
